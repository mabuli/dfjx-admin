...
import io.dfjx.module.sys.service.SysDeptService;
...
public class SysDeptController extends AbstractController {
	...
	@RequestMapping("/lable/{deptId}")
	public R lable(@PathVariable("deptId") Long deptId){
		List<SysDeptEntity> list = new ArrayList<>();
		if (deptId != 0) {
			getLable(list ,deptId);
		}

		return R.ok().put("list", list);
	}

	private void getLable(List<SysDeptEntity> list, Long deptId){
		SysDeptEntity dept = sysDeptService.getById(deptId);
		list.add(0, dept);
		if (dept.getParentId() != 0) {
			getLable(list, dept.getParentId());
		}
	}
